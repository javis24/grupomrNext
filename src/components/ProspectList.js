        ...
        <div className="p-4 md:p-8 bg-[#0e1624] text-white min-h-screen font-sans">
            ...
            <div className="max-w-5xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                    ...
                    <button 
                        ...
                        className="bg-blue-600 hover:bg-blue-500 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-lg flex items-center gap-2"
                    >
                ...
                {/* Filtros */}
                <div className="bg-[#1f2937] p-4 rounded-3xl border border-gray-700 shadow-2xl mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="text" placeholder="Buscar por nombre..." className="bg-[#0e1624] border border-gray-700 p-3 rounded-xl text-sm outline-none focus:border-blue-500 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    <input type="text" placeholder="Filtrar por etapa..." className="bg-[#0e1624] border border-gray-700 p-3 rounded-xl text-sm outline-none focus:border-blue-500 transition-all" value={searchProcess} onChange={(e) => setSearchProcess(e.target.value)} />
                </div>

                {/* Listado */}
                <div className="space-y-4">
                    {filteredProspects.map((prospect) => (
                        <div key={prospect.id} className="bg-[#1f2937] p-6 rounded-[2rem] border border-gray-700 flex flex-col md:flex-row justify-between items-center gap-4 hover:border-blue-500/50 transition-all group">
                            <div className="flex flex-col items-center md:items-start text-center md:text-left">
                                <h3 className="text-xl font-black text-white uppercase tracking-tight group-hover:text-blue-400 transition-colors">{prospect.contactName}</h3>
                                <p className="text-blue-500 text-[10px] font-bold uppercase tracking-[0.2em] mb-2">{prospect.company}</p>
                                <div className="flex flex-wrap justify-center md:justify-start gap-4 text-[10px] text-gray-500 font-bold uppercase">
                                    <span>⚙️ {prospect.saleProcess}</span>
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => exportToPDF(prospect)} className="p-3 bg-gray-800 rounded-xl text-red-500 hover:bg-red-500 hover:text-white transition-all"><FiFileText size={18}/></button>
                                <button onClick={() => { setEditingProspect(prospect); setFormData({...prospect}); setModalType("editProspect"); setModalIsOpen(true); }} className="p-3 bg-gray-800 rounded-xl text-yellow-500 hover:bg-yellow-500 hover:text-black transition-all"><FiEdit size={18}/></button>
                                {userRole === 'admin' && <button onClick={() => handleDelete(prospect.id)} className="p-3 bg-gray-800 rounded-xl text-gray-500 hover:bg-red-600 hover:text-white transition-all"><FiX size={18}/></button>}
                                {prospect.saleProcess === "Cerrado" && (
                                    <button onClick={() => handleConvertClick(prospect)} className="bg-green-600 hover:bg-green-500 px-4 py-3 rounded-xl font-black text-[9px] uppercase tracking-wider flex items-center gap-2 transition-all shadow-lg shadow-green-900/20 animate-pulse">
            ...
            <Modal isOpen={modalIsOpen} onRequestClose={() => setModalIsOpen(false)} style={customStyles}>
                <div className="flex justify-between items-center mb-8 border-b border-gray-700 pb-4">
                    <h2 className="text-2xl font-black text-blue-500 uppercase italic">
                        {modalType === "convertToClient" ? "💎 Alta de Cliente" : (modalType === "createProspect" ? "📝 Nuevo Prospecto" : "✏️ Editar")}
                    </h2>
                    <button onClick={() => setModalIsOpen(false)} className="text-gray-500 hover:text-white transition-colors"><FiX size={32}/></button>
                </div>
                ...
                <form onSubmit={modalType === "convertToClient" ? handleClientSave : handleSubmit} className="space-y-6">
                    {modalType === "convertToClient" ? (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="space-y-4">
                                <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest border-l-4 border-blue-500 pl-2">Empresa</p>
                            ...
                            <div className="space-y-4">
                                <p className="text-[10px] font-black text-green-400 uppercase tracking-widest border-l-4 border-green-500 pl-2">Contacto</p>
                                ...
                                <div className="flex flex-col gap-1">
                                    <label className="text-[10px] text-gray-500 font-bold uppercase ml-1">Planta / Unidad</label>
                                    <select value={newClient.planta} onChange={(e) => setNewClient({...newClient, planta: e.target.value})} className="bg-[#0e1624] border border-gray-700 p-3 rounded-xl text-sm text-white" required>
                                        <option value="">Seleccionar Unidad...</option>
                                        {unidadesNegocio.map(u => <option key={u} value={u}>{u}</option>)}
                                    </select>
                                </div>
                                <div className="flex flex-col gap-1">
                                    <label className="text-[10px] text-gray-500 font-bold uppercase ml-1">Asesor Responsable</label>
                                    <select disabled={userRole !== 'admin'} value={newClient.assignedUser} onChange={(e) => setNewClient({...newClient, assignedUser: e.target.value})} className="bg-[#0e1624] border border-gray-700 p-3 rounded-xl text-sm text-white" required>
                            ...
                            <div className="space-y-4 bg-gray-800/30 p-4 rounded-2xl border border-gray-700">
                                <p className="text-[10px] font-black text-yellow-400 uppercase tracking-widest">Facturación</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex flex-col gap-1">
                                <label className="text-[10px] text-gray-500 font-bold uppercase ml-1">Etapa actual</label>
                                <select name="saleProcess" value={formData.saleProcess} onChange={handleInputChange} className="bg-[#0e1624] border border-gray-700 p-4 rounded-xl text-sm text-white" required>
                    ...
                    <div className="flex gap-4 pt-8 border-t border-gray-700">
                        <button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-500 py-4 rounded-2xl font-black uppercase text-xs tracking-[0.2em] shadow-xl transition-all active:scale-95">
                            {modalType === "convertToClient" ? "🚀 Finalizar Registro" : "💾 Guardar Cambios"}
                        </button>
                        <button type="button" onClick={() => setModalIsOpen(false)} className="px-8 bg-gray-800 text-gray-400 py-4 rounded-2xl font-black uppercase text-xs transition-all">Cancelar</button>
                    </div>
                </form>
            </Modal>
        </div>
...
const ModalInput = ({ label, value, onChange, type = "text", required = false }) => (
    <div className="flex flex-col gap-1">
        <label className="text-[10px] text-gray-500 font-bold uppercase ml-1">{label} {required && "*"}</label>
        <input 
            type={type} 
            value={value || ''} 
            onChange={(e) => onChange(e.target.value)} 
            required={required}
            className="bg-[#0e1624] border border-gray-700 p-3 rounded-xl text-sm text-white outline-none focus:border-blue-500 transition-all" 
        />
    </div>
);