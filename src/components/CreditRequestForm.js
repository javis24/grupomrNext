...
const CreditRequestForm = () => {
    ...
    const handleSave = async () => {
        if (!formData.nombreComercial || !formData.rfc) {
            return toast.error('Nombre y RFC obligatorios');
        }
    ...
    const generatePDF = (rawData = formData) => {
        ...
        doc.autoTable({
            ...
            body: [
                [
                    'COMPRAS',
                    data.contactos?.compras?.nombre || '',
                    data.contactos?.compras?.tel || '',
                    data.contactos?.compras?.correo || '',
                ],
                [
                    'PAGOS',
                    data.contactos?.pagos?.nombre || '',
                    data.contactos?.pagos?.tel || '',
                    data.contactos?.pagos?.correo || '',
                ],
            ],
            styles: { fontSize: 7 },
        });
    ...
    const filteredCredits = savedCredits.filter(
        (c) =>
