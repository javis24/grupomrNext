...
const CreateQuote = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 dark:bg-[#0e1624] dark:text-white p-4 md:p-8 font-sans flex flex-col gap-10 transition-colors duration-300">
      ...
      {/* CARD PRINCIPAL GENERADOR */}
      <div className="max-w-7xl mx-auto w-full bg-white dark:bg-[#1f2937] p-6 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-2xl transition-colors">
        <h1 className="text-2xl font-black text-blue-600 dark:text-blue-400 uppercase mb-8 border-b border-gray-100 dark:border-gray-700 pb-4 flex justify-between items-center">
          {originalQuoteLoaded ? "Modificando Cotización" : "Generador de Cotización"} 
          <span className="text-yellow-600 dark:text-yellow-500 font-mono">#{originalQuoteLoaded ? originalQuoteLoaded.quoteNumber : quoteNumber}</span>
        </h1>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Campo Empresa con Sugerencias */}
            <div className="flex flex-col gap-1 relative" ref={suggestionsRef}>
              <label className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase ml-1">Empresa</label>
              <input 
                type="text" 
                name="companyName" 
                autoComplete="off" 
                ...
                className="bg-gray-100 dark:bg-[#0e1624] p-3 rounded-xl border border-gray-300 dark:border-gray-700 text-sm text-gray-900 dark:text-white outline-none focus:border-blue-500 transition-all" 
              />
              {showSuggestions && filteredClients.length > 0 && (
                <div className="absolute top-full left-0 w-full bg-white dark:bg-[#1f2937] border border-gray-200 dark:border-gray-600 rounded-xl mt-1 z-50 shadow-2xl max-h-60 overflow-y-auto">
                  {filteredClients.map((client) => (
                    <div key={client.id} onClick={() => selectClient(client)} className="p-3 hover:bg-blue-600 hover:text-white cursor-pointer border-b border-gray-100 dark:border-gray-700 last:border-0 text-gray-700 dark:text-gray-200">
                      <div className="text-sm font-bold">{client.companyName || client.fullName}</div>
                      <div className="text-[10px] uppercase opacity-70">{client.contactName}</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase ml-1 tracking-widest">Descripción del Producto / Servicio</label>
            <textarea 
              value={descripcionGeneral} 
              onChange={(e) => setDescripcionGeneral(e.target.value)} 
              placeholder="Escriba la descripción..." 
              className="bg-gray-100 dark:bg-[#0e1624] p-4 rounded-2xl border border-gray-300 dark:border-gray-700 text-sm text-gray-900 dark:text-white outline-none focus:border-blue-500 transition-all h-full min-h-[150px] resize-none" 
            />
          </div>
        </div>
          ...
          {serviceRows.map((row, index) => (
            <div key={index} className="bg-gray-50 dark:bg-[#0e1624]/50 p-4 rounded-2xl border border-gray-200 dark:border-gray-800 grid grid-cols-1 md:grid-cols-12 gap-3 items-end transition-colors">
              <div className="md:col-span-3 flex flex-col gap-1">
                <label className="text-[9px] font-bold text-gray-400 dark:text-gray-600 uppercase">Descripción</label>
                <input type="text" name="description" value={row.description} onChange={(e) => handleRowChange(index, e)} className="bg-white dark:bg-[#1f2937] p-2 rounded-lg border border-gray-300 dark:border-gray-700 text-sm text-gray-900 dark:text-white outline-none" />
              </div>
              <div className="md:col-span-1 flex flex-col gap-1">
                <label className="text-[9px] font-bold text-gray-400 dark:text-gray-600 uppercase text-center">U.M.</label>
                <input type="text" name="um" value={row.um} onChange={(e) => handleRowChange(index, e)} className="bg-white dark:bg-[#1f2937] p-2 rounded-lg border border-gray-300 dark:border-gray-700 text-sm text-center uppercase text-gray-900 dark:text-white" />
              </div>
              <div className="md:col-span-1 flex flex-col gap-1">
                <label className="text-[9px] font-bold text-gray-400 dark:text-gray-600 uppercase text-center">P.U.</label>
                <input type="number" name="pu" value={row.pu} onChange={(e) => handleRowChange(index, e)} className="bg-white dark:bg-[#1f2937] p-2 rounded-lg border border-gray-300 dark:border-gray-700 text-sm text-green-600 dark:text-green-400 font-bold text-center" />
              </div>
              <div className="md:col-span-1 flex flex-col gap-1">
                <label className="text-[9px] font-bold text-gray-400 dark:text-gray-600 uppercase text-center">Cant.</label>
                <input type="number" name="cantidad" value={row.cantidad} onChange={(e) => handleRowChange(index, e)} className="bg-white dark:bg-[#1f2937] p-2 rounded-lg border border-gray-300 dark:border-gray-700 text-sm text-center font-bold text-gray-900 dark:text-white" />
              </div>
              <div className="md:col-span-1 flex flex-col gap-1">
              ...
              <div className="md:col-span-4 flex flex-col gap-1">
                ...
                <input 
                  type="text" 
                  name="comments" 
                  placeholder="Observaciones por fila..." 
                  value={row.comments || ''} 
                  onChange={(e) => handleRowChange(index, e)} 
                  className="bg-white dark:bg-[#1f2937] p-2 rounded-lg border border-gray-300 dark:border-gray-700 text-xs text-gray-700 dark:text-gray-300 outline-none focus:border-yellow-500/50" 
                />
              </div>
              <div className="md:col-span-1">
                <button type="button" onClick={() => removeRow(index)} className="w-full bg-red-100 dark:bg-red-600/10 text-red-600 dark:text-red-500 p-2 rounded-lg hover:bg-red-600 hover:text-white transition-all"><FiTrash2 size={16} className="mx-auto"/></button>
              </div>
            </div>
          ))}
          <button type="button" onClick={addRow} className="flex items-center gap-2 text-[10px] font-black text-blue-600 dark:text-blue-500 uppercase hover:text-blue-700 dark:hover:text-blue-400 transition-colors">
            <FiPlus /> Agregar Producto/Servicio
          </button>
        </div>

        {/* OBSERVACIONES Y TÉRMINOS */}
        <div className="mb-10 bg-gray-50 dark:bg-[#0e1624]/30 p-6 rounded-[2rem] border border-gray-200 dark:border-gray-800">
          <h2 className="text-sm font-black text-yellow-600 dark:text-yellow-500 uppercase tracking-widest mb-4 flex items-center gap-2"><FiFileText /> Términos y Observaciones</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {opcionesObservaciones.map((opcion, i) => (
              <div key={i} onClick={() => toggleObservacion(opcion)} 
                className={`flex items-start gap-3 p-3 rounded-xl cursor-pointer transition-all border ${
                  observacionesSeleccionadas.includes(opcion) 
                  ? 'bg-blue-100 dark:bg-blue-600/20 border-blue-400 dark:border-blue-500 text-blue-700 dark:text-blue-400 shadow-md' 
                  : 'bg-white dark:bg-[#1f2937]/50 border-gray-200 dark:border-gray-700 text-gray-500 hover:border-gray-400'
                }`}>
        ...
        {/* TOTALES FINALES */}
        <div className="mt-8 flex flex-col items-end gap-2 border-t border-gray-100 dark:border-gray-700 pt-8">
            <div className="text-gray-500 dark:text-gray-400 text-sm font-bold uppercase">Subtotal: <span className="text-gray-900 dark:text-white ml-2">$ {globalSubtotal}</span></div>
            <div className="text-gray-500 dark:text-gray-400 text-sm font-bold uppercase">IVA (16%): <span className="text-yellow-700 dark:text-yellow-600 ml-2">$ {globalIva}</span></div>
            <div className="text-4xl font-black text-gray-900 dark:text-white mt-2">TOTAL: <span className="text-green-600 dark:text-green-500 font-mono">${globalTotal}</span></div>
        </div>

        <div className="mt-8 flex justify-end gap-4">
          {originalQuoteLoaded && (
            <button onClick={() => { setOriginalQuoteLoaded(null); setServiceRows([{ description: '', cantidad: 1, um: 'SERVICIO', pu: 0, subtotal: 0, iva: 0, total: 0, comments: '' }]); }} className="px-8 py-4 bg-red-100 dark:bg-red-600/10 text-red-600 dark:text-red-500 rounded-2xl font-black text-xs uppercase transition-all">
              Cancelar Edición
            </button>
          )}
          <button onClick={generatePDF} disabled={isSaving} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-lg shadow-blue-900/40">
            {isSaving ? "Procesando..." : originalQuoteLoaded ? "Guardar Nueva Versión" : "Descargar y Guardar"}
          </button>
          <button onClick={() => router.back()} className="px-8 py-4 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 rounded-2xl font-black text-xs uppercase text-gray-600 dark:text-gray-400 transition-all">Cerrar</button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto w-full bg-white dark:bg-[#1f2937] p-6 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-2xl transition-colors">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl font-black text-gray-800 dark:text-white uppercase tracking-widest flex items-center gap-2"><FiFileText className="text-blue-600 dark:text-blue-400" /> Historial Reciente</h2>
          <div className="relative w-full md:w-96">
            <FiSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
            <input 
              type="text" 
              placeholder="Buscar por Empresa o Producto..." 
              className="w-full bg-gray-50 dark:bg-[#0e1624] border border-gray-200 dark:border-gray-700 rounded-2xl py-3 pl-12 pr-4 text-sm text-gray-900 dark:text-white outline-none focus:border-blue-500 transition-all" 
        ...
        <div className="overflow-x-auto rounded-2xl border border-gray-100 dark:border-gray-800">
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-100 dark:bg-[#0e1624] text-gray-500 dark:text-gray-400 font-black uppercase text-[10px] tracking-widest">
              <tr>
