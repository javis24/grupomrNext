            ...
            <div className="max-w-7xl mx-auto">
                ...
                {!selectedCategory ? (
                    <div className="animate-in fade-in duration-500">
                        ...
                        <p className="text-blue-400 text-xs font-bold tracking-[0.4em] uppercase mb-10">Unidad Estratégica de Negocio</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                            {categories.map((cat) => (
                                <div key={cat.name} onClick={() => handleCategorySelect(cat.name)}
                                    className="bg-[#1f2937] p-10 rounded-[2.5rem] border border-gray-700 hover:border-blue-500 cursor-pointer transition-all hover:translate-y-[-8px] shadow-2xl group relative overflow-hidden h-64 flex flex-col justify-center"
                                >
                                    ...
                                    <div className="relative z-10 text-center md:text-left">
                                        ...
                                        <div className="flex items-center justify-center md:justify-start gap-3">
                                            ...
                                            <span className="text-blue-400 font-bold text-xs uppercase tracking-widest">{products.filter(p => p.businessUnit === cat.name).length} PRODUCTOS</span>
                                        </div>
                                    </div>
                    ...
                    <div className="animate-in slide-in-from-bottom-5 duration-500">
                        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
                            <div className="flex items-center gap-5">
                                <button onClick={() => setSelectedCategory(null)} className="bg-[#1f2937] p-4 rounded-2xl hover:bg-blue-600 transition-all border border-gray-700">
                                    <FiArrowLeft size={24} />
                                </button>
                            ...
                            <form onSubmit={handleSubmit} className="bg-[#1f2937] p-8 rounded-[2.5rem] border border-gray-700 mb-10 shadow-2xl space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <select value={formData.unitMeasure} onChange={(e)=>setFormData({...formData, unitMeasure: e.target.value})} className="bg-[#0e1624] border border-gray-700 rounded-2xl p-4 text-sm text-white outline-none">
                                    ...
                                    <div className="md:col-span-3">
                                        <label className="text-[10px] font-black text-gray-500 uppercase ml-2 tracking-widest">Descripción</label>
                                        <textarea rows="3" value={formData.description} onChange={(e)=>setFormData({...formData, description: e.target.value})} className="w-full bg-[#0e1624] border border-gray-700 rounded-2xl p-4 text-sm text-white outline-none focus:border-blue-500 resize-none" />
                                    </div>
                                </div>
                                <button type="submit" className="w-full bg-blue-600 py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-blue-500 active:scale-95 transition-all">
                                    {isEditing ? 'Guardar Cambios' : 'Registrar SKU'}
                                </button>
                            </form>
                        )}

                        <div className="bg-[#1f2937] p-5 rounded-3xl border border-gray-700 flex items-center shadow-2xl focus-within:ring-2 ring-blue-500/50 mb-10">
                            <FiSearch className="text-gray-500 mx-4" size={24} />
                            <input type="text" placeholder={`Buscar en ${selectedCategory}...`} className="bg-transparent w-full outline-none text-lg text-white" onChange={(e) => setSearchTerm(e.target.value)} />
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                            {filteredProducts.map(product => (
                                <div key={product.id} className="bg-[#1f2937] rounded-[2.5rem] border border-gray-700 p-8 hover:border-blue-500/50 transition-all shadow-xl group">
                                    <div className="flex justify-between items-start mb-6">
                                        <div className="bg-blue-500/10 p-4 rounded-2xl text-blue-500 text-3xl"><FiPackage /></div>
                                        {userRole === 'admin' && (
                                    ...
                                    <p className="text-blue-500 text-[11px] font-mono font-bold mb-4 uppercase tracking-wider">ID: {product.code || 'S/N'}</p>
                                    <div className="space-y-4 border-t border-gray-800 pt-6">
                                        <p className="text-sm text-gray-400 italic line-clamp-2 h-10">{product.description || 'Sin descripción.'}</p>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="bg-[#0e1624] p-3 rounded-2xl border border-gray-800">
                                                <p className="text-[8px] text-gray-600 font-black uppercase">Precio</p>
                                            ...
                                            <div className="bg-[#0e1624] p-3 rounded-2xl border border-gray-800 text-center">
                                                <p className="text-[8px] text-gray-600 font-black uppercase">Medida</p>
                                                <p className="text-xs font-bold text-gray-300 mt-1 uppercase italic">{product.unitMeasure}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
const ModalInput = ({ label, value, onChange, type = "text", placeholder = "", required = false }) => (
    <div className="flex flex-col gap-2">
        ...
        <input type={type} placeholder={placeholder} required={required} value={value || ''} onChange={(e)=>onChange(e.target.value)} 
               className="bg-[#0e1624] border border-gray-700 rounded-2xl p-4 text-sm text-white outline-none focus:border-blue-500 transition-all" />
    </div>
);
